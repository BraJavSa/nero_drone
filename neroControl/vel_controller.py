...
ACCEL_FF = 0.15  # feedforward de aceleración — ajustar entre 0.1-0.3
...
class PIDChannel:
    ...
    def update(self, e: float, measurement: float, dt: float) -> float:
        ...
        # Derivative on measurement — evita derivative kick cuando cambia la referencia
...
class BebopVelocityController(Node):
    def __init__(self):
        ...
        # Estado previo de referencia en body frame para feedforward de aceleración
    ...
    def is_flying_callback(self, msg: Bool):
        if not msg.data and self.is_flying:
            [p.reset() for p in self.pid.values()]
            self.ref_vx_b_prev = self.ref_vy_b_prev = 0.0
    ...
    def control_loop(self):
        ...
        # Feedforward de aceleración — anticipa cambios en la referencia
        d_ref_vx_b = (ref_vx_b - self.ref_vx_b_prev) / self.dt
        d_ref_vy_b = (ref_vy_b - self.ref_vy_b_prev) / self.dt
        self.ref_vx_b_prev = ref_vx_b
        self.ref_vy_b_prev = ref_vy_b
        ...
        # Decaimiento suave del integral al parar — evita saltos bruscos
        decay = 0.95
        if abs(self.ref_vx_i) < 0.05: self.pid['x']._e_int *= decay
        if abs(self.ref_vy_i) < 0.05: self.pid['y']._e_int *= decay

        ff_w = 1.3
        ux   = (ff_w * ref_vx_b        / self.vx_ss)   + ACCEL_FF * d_ref_vx_b + self.pid['x'].update(ex_b,   self.vx_b,   self.dt)
        uy   = (ff_w * ref_vy_b        / self.vy_ss)   + ACCEL_FF * d_ref_vy_b + self.pid['y'].update(ey_b,   self.vy_b,   self.dt)
        uz   = (ff_w * self.ref_vz_i   / self.vz_ss)                           + self.pid['z'].update(ez_b,   self.vz_b,   self.dt)
        uyaw = (ff_w * self.ref_dyaw_i / self.dyaw_ss)                         + self.pid['yaw'].update(eyaw_b, self.dyaw_b, self.dt)
