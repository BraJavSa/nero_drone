#!/usr/bin/env python3
# UAV position and attitude controller using ROS2
import rclpy
    ...
    def cController(self):
        ...
        if self.base_opt == 2:
            if self.opt == 2 and error_posicion > 0.40:
                ...
                self.node.get_logger().info(f"High error ({error_posicion:.2f}m > 0.40m): Switching to opt=3")
                ...
                if hasattr(self, 'saved_opt2_params'):
                    ...
                    self.node.set_parameters(new_params)
                    self.node.get_logger().info(f"Low error ({error_posicion:.2f}m < 0.05m): Reverting to opt=2")
