#!/usr/bin/env python3
"""
System identification validator for Bebop 2 drone.
Supports both continuous-time (f1, f2) and discrete-time (alpha, beta) formats.
"""
...
def fit_percentage(y_true, y_pred):
    # NRMSE fit percentage used in coupled_dynamics_identifier.py
...
def main():
    # ─── 1. Load system identification results ─────────────────────────────────
    json_path = "system_identification_results.json"
    if not os.path.exists(json_path):
        print(f"[ERROR] Parameter file '{json_path}' not found.")
        return

    with open(json_path, 'r') as f:
        config = json.load(f)
    
    params = config["identified_parameters"]
    delays = config.get("delays", {})
    delay_x = delays.get("x", 2)
    delay_y = delays.get("y", 2)
    ...
    # Discrete-time form: f1 (alpha) ~ 1.0, f2 (beta) ~ 0.0
    # Continuous-time form: f1 (input gain) ~ 0.1-2.0, f2 (damping) ~ 0.1-5.0
    channels = ["x", "y", "z", "yaw"]
    f1_cont = {}
    f2_cont = {}
    is_discrete = {}
    ...
    for ch in channels:
        f1_val = params[ch]["f1"]
        f2_val = params[ch]["f2"]
        # If f1 is large (e.g. > 0.5) and f2 is small (e.g. < 0.2), it's discrete
        if f1_val > 0.5 and f2_val < 0.2:
            is_discrete[ch] = True
            # f1_val is alpha (discrete state transition), f2_val is beta (discrete input gain)
            # v[k+1] = alpha * v[k] + beta * u[k]
            # Continuous equivalent: f1_cont = beta * hz, f2_cont = (1 - alpha) * hz
            f1_cont[ch] = f2_val * hz
            f2_cont[ch] = (1.0 - f1_val) * hz
        else:
            is_discrete[ch] = False
            # Already continuous f1, f2
            f1_cont[ch] = f1_val
            f2_cont[ch] = f2_val

    print("="*60)
    print("LOADED PARAMETERS & FORMAT AUTO-DETECTION")
    print("="*60)
    for ch in channels:
        fmt = "DISCRETE (alpha/beta)" if is_discrete[ch] else "CONTINUOUS (f1/f2)"
        print(f"{ch.upper()} channel loaded as: {fmt}")
        print(f"  Loaded values:  f1 = {params[ch]['f1']:.6f} | f2 = {params[ch]['f2']:.6f}")
        print(f"  Continuous eq:  f1 = {f1_cont[ch]:.6f} | f2 = {f2_cont[ch]:.6f}")
        alpha_eq = 1.0 - dt * f2_cont[ch]
        beta_eq = dt * f1_cont[ch]
        print(f"  Discrete eq:    alpha = {alpha_eq:.6f} | beta = {beta_eq:.6f}")
    print(f"Delays: X = {delay_x} | Y = {delay_y}")
    print("="*60)
    ...
    # ─── 3. Filter Z and Yaw positions (Causal Moving Average W=5) ────────────
    ...
    # ─── 4. Apply channel-specific delays to X and Y control commands ─────────
    u_delayed = u_t.copy()
    ...
    # ─── 5. Simulación forward conjunta ───────────────────────────────────────
    f1_vec = np.array([f1_cont["x"], f1_cont["y"], f1_cont["z"], f1_cont["yaw"]])
    f2_vec = np.array([f2_cont["x"], f2_cont["y"], f2_cont["z"], f2_cont["yaw"]])
    ...
    for k in range(M_samples - 1):
        ...
        u_k[2] = u_t[k, 2] # z is not delayed
        u_k[3] = u_t[k, 3] # yaw is not delayed
    ...
    for k in range(M_samples - 1):
        ...
        pos_sim[k+1, 2] = pos_sim[k, 2] + 0.5 * (vzb + nu_sim[k+1, 2]) * dt # Trapezoidal
        pos_sim[k+1, 3] = pos_sim[k, 3] + 0.5 * (om + nu_sim[k+1, 3]) * dt # Trapezoidal
    ...
    fig1.suptitle("Velocity Validation (Simulated vs Real Telemetry)", fontsize=14)
    vel_names = [r"$v_{x,b}$ [m/s]", r"$v_{y,b}$ [m/s]", r"$v_{z,b}$ [m/s]", r"$\omega$ [rad/s]"]
    
    for i in range(4):
        axes1[i].plot(t, nu_real[:, i], label='Real Telemetry', color='dodgerblue', alpha=0.8)
        axes1[i].plot(t, nu_sim[:, i], '--', label='Simulated Model', color='crimson')
        axes1[i].set_ylabel(vel_names[i])
        axes1[i].legend(loc='upper right')
        axes1[i].grid(True, alpha=0.3)
    axes1[-1].set_xlabel("Time [s]")
    plt.tight_layout()
    fig1.savefig("validation_velocities.png", dpi=150)

    fig2, axes2 = plt.subplots(4, 1, figsize=(12, 10), sharex=True)
    fig2.suptitle("Position Validation (Simulated vs Real Telemetry)", fontsize=14)
    pos_names = ["x [m]", "y [m]", "z [m]", r"$\psi$ [rad]"]
    pos_reals = [x_real, y_real, z_real, psi_real]
    
    for i in range(4):
        axes2[i].plot(t, pos_reals[i], label='Real Telemetry', color='dodgerblue', alpha=0.8)
        axes2[i].plot(t, pos_sim[:, i], '--', label='Simulated Model', color='crimson')
        axes2[i].set_ylabel(pos_names[i])
        axes2[i].legend(loc='upper right')
        axes2[i].grid(True, alpha=0.3)
    axes2[-1].set_xlabel("Time [s]")
    plt.tight_layout()
    fig2.savefig("validation_positions.png", dpi=150)
