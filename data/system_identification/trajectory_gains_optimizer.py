...
# Evaluates and optimizes tracking gains across dynamic reference trajectories.
"""
Bebop Controller Optimizer
- Trayectoria: lemniscata (idéntica al nodo ROS2)
...
Ad = np.array([
    [ 0.978268, -0.005937,  0.000000,  0.003408],
    [-0.002204,  0.956074,  0.000000,  0.008881],
    [ 0.000000,  0.000000,  0.857642,  0.000000],
    [ 0.000000,  0.000000,  0.000000,  0.822490],
])
Bd = np.array([
    [ 0.040904,  0.006245,  0.000000, -0.004634],
    [ 0.001016,  0.041965,  0.000000, -0.012598],
    [ 0.000000,  0.000000,  0.180292,  0.000000],
    [ 0.000000,  0.000000,  0.000000,  0.184677],
])

DT        = 1.0 / 15.0
OPT_TIME  = 42.0
FULL_TIME = 63.0
OPT_STEPS  = int(OPT_TIME  / DT)
FULL_STEPS = int(FULL_TIME / DT)
...
GAIN_DEFAULTS = np.array([
    1.20, 1.20, 1.2527, 3.00,
    0.70, 0.70, 2.5484, 1.2455,
    1.95, 1.95, 1.5747, 2.00,
    0.00, 0.00, 0.00,   0.00,
])
...
def compute_lemniscate(t_arr, w, ax, ay, zb, za):
    x  = ax * np.sin(w * t_arr)
    y  = ay * np.sin(w * t_arr) * np.cos(w * t_arr)
    z  = zb + za * np.sin(0.5 * w * t_arr)
    dx = ax * w * np.cos(w * t_arr)
    dy = ay * w * np.cos(2.0 * w * t_arr)
    dz = za * 0.5 * w * np.cos(0.5 * w * t_arr)
    raw_yaw = np.arctan2(dy, dx)
    yaw  = np.unwrap(raw_yaw)
    dyaw = np.empty_like(yaw)
    dyaw[1:-1] = (yaw[2:] - yaw[:-2]) / (2*DT)
    dyaw[0]    = (yaw[1]  - yaw[0])   / DT
    dyaw[-1]   = (yaw[-1] - yaw[-2])  / DT
    xd  = np.column_stack([x,  y,  z,  yaw])
    dxd = np.column_stack([dx, dy, dz, dyaw])
    return xd, dxd
...
def run_simulation(gains_vec, xd_arr, dxd_arr, n_steps):
    """
    gains_vec : array 16 ganancias en orden GAIN_NAMES
    Devuelve  : dict con arrays (n_steps, 4)
    """
    g = gains_vec
    ksp = np.diag(g[0:4])
    ksd = np.diag(g[4:8])
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Bebop Controller Optimizer — Lemniscata · 15 Hz")
    ...
    def _build_ui(self):
        ...
        tk.Label(hdr, text="lemniscata · Ad/Bd · 15 Hz",
    ...
    def _build_left_panel(self):
        ...
        def make_entry(parent, label, var, lo, hi):
            ...
            sl = ttk.Scale(row, from_=lo, to=hi, orient='horizontal',
                           variable=var)
            sl.pack(fill='x')
            def _upd(*_, v=var, l=lbl_v):
                l.config(text=f"{v.get():.3f}")
            var.trace_add('write', _upd)

        section("TRAYECTORIA", VIOLET)
        traj_defs = [('w',     0.02, 0.40, 0.10),
                     ('amp_x', 0.20, 3.00, 1.50),
                     ('amp_y', 0.20, 3.00, 1.50),
                     ('z_base',0.30, 3.00, 1.50),
                     ('z_amp', 0.00, 1.50, 0.50)]
        for n, lo, hi, d in traj_defs:
            v = tk.DoubleVar(value=d)
            self._traj_vars[n] = v
            make_entry(inner, n, v, lo, hi)
    ...
    def _copy_params(self):
        ...
        for i, n in enumerate(GAIN_NAMES):
            lines.append(f"{n} = {self._best_gains[i]:.6f}")
    ...
    def _start_opt(self):
        ...
        tp = self._get_traj()
        w  = self._get_weights()
        xd, dxd = compute_lemniscate(T_OPT,
                                     tp['w'], tp['amp_x'], tp['amp_y'],
                                     tp['z_base'], tp['z_amp'])
    ...
    def _show_full_sim(self):
        tp = self._get_traj()
        xd, dxd = compute_lemniscate(T_FULL,
                                     tp['w'], tp['amp_x'], tp['amp_y'],
                                     tp['z_base'], tp['z_amp'])
        h  = run_simulation(self._best_gains, xd, dxd, FULL_STEPS)
        t  = T_FULL
        posR_h = xd
        velR_h = np.zeros((FULL_STEPS, 4))
        vl = ['vx','vy','vz','r']
        ...
        for i,l in enumerate(LABELS):
            ...
            ax0.plot(t, posR_h[:,i],  color=COLORS[i], lw=0.7,
