...
# ─────────────────────────────────────────────
# 1. CARGA Y PREPROCESAMIENTO DE LOS 3 DATASETS
# ─────────────────────────────────────────────
...
def load_and_preprocess(mat_path, skip_samples=0, dt=1/30.0, window_size=30):
    ...
    # Eliminar muestras iniciales si se pide
    ...
    # Derivada numérica → aceleraciones
    dv = np.diff(v_smooth, axis=0) / dt

    # ── Construcción del vector de features ──────────────────────────
    # Antes: [v_hist(W×4), u_hist(W×4), u_now(4)]  → 244 features
    # Ahora: [v_now(4),    u_hist(W×4), u_now(4)]  → 128 features
    # ─────────────────────────────────────────────────────────────────
    X_list, Y_list = [], []
    for t in range(window_size, len(dv)):
        v_now     = v_smooth[t].flatten()                      # 4
        u_history = u_raw[t - window_size:t].flatten()        # W*4
        u_actual  = u_raw[t].flatten()                        # 4
...
# ─────────────────────────────────────────────
# 6. VALIDACIÓN RK4  (un plot por dataset)
# ─────────────────────────────────────────────
