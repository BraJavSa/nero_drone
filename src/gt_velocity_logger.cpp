// High-performance C++ node for logging synchronized ground truth odometry and velocity commands.
...
class GtTopicLogger : public rclcpp::Node {
public:
  GtTopicLogger()
      : Node("gt_topic_logger_node"), first_sample_(true), ref_first_sample_(true),
    ...
    RCLCPP_INFO(this->get_logger(), "GT Logger node started");

    sub_odom_ = this->create_subscription<nav_msgs::msg::Odometry>(
        "/bebop/gt_fullodom", 10,
        std::bind(&GtTopicLogger::odomCallback, this, std::placeholders::_1));

    sub_cmd_ = this->create_subscription<geometry_msgs::msg::Twist>(
        "/bebop/cmd_vel", 10,
        std::bind(&GtTopicLogger::cmdCallback, this, std::placeholders::_1));

    sub_ref_ = this->create_subscription<std_msgs::msg::Float64MultiArray>(
        "/bebop/ref_vec", 10,
        std::bind(&GtTopicLogger::refCallback, this, std::placeholders::_1));
    ...
    filename << base_path << "/gt_bebop_log_"
    ...
    RCLCPP_INFO(this->get_logger(), "Logging GT to: %s", filename.str().c_str());
    ...
    timer_ = this->create_wall_timer(std::chrono::duration<double>(1.0 / 30.0),
                                     std::bind(&GtTopicLogger::logData, this));
  }

  ~GtTopicLogger() { logfile_.close(); }
...
int main(int argc, char **argv) {
  ...
  auto node = std::make_shared<GtTopicLogger>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
