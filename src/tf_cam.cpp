...
public:
  CameraTFWithGimbal() : Node("camera_tf_with_gimbal") {
    ...
    sub_move_camera_ = this->create_subscription<geometry_msgs::msg::Vector3>(
        ...
        std::bind(&CameraTFWithGimbal::move_camera_callback, this,
                  std::placeholders::_1));
    ...
    timer_ = this->create_wall_timer(
        20ms, std::bind(&CameraTFWithGimbal::update_camera_pitch, this));
...
private:
  ...
  void update_camera_pitch() {
    ...
    // Intentar obtener la TF con timeout explícito para no tragarse errores
    // silenciosamente
    geometry_msgs::msg::TransformStamped tf_base;
    try {
      tf_base = tf_buffer_->lookupTransform(
          "odom", "base_link", tf2::TimePointZero,
          tf2::durationFromSec(0.01) // espera hasta 10 ms antes de lanzar
      );
    } catch (const tf2::TransformException &ex) {
      RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
                           "lookupTransform odom→base_link falló: %s",
                           ex.what());
      return;
    }
  ...
  // Las tres transformadas se calculan y envían en una sola llamada al
  // broadcaster
  void publish_all_tfs(double pitch_angle,
    ...
    // 1) absolute_cam_link — frame estático de montaje (relativo a
    // base_link_ekf)
    auto &tf_abs = tfs[0];
    ...
    // 3) camera_gimbal — compensación de roll/pitch del cuerpo (relativo a
    // camera_link)
