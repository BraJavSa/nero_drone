// Computes and broadcasts the virtual gimbal and camera coordinate transformations.
...
private:
  ...
  void update_camera_pitch() {
    ...
    try {
      tf_base = tf_buffer_->lookupTransform(
          "odom", "bebop_link", tf2::TimePointZero,
          tf2::durationFromSec(0.01) 
      );
    } catch (const tf2::TransformException &ex) {
      RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
                           "lookupTransform odom->base_link failed: %s",
                           ex.what());
      return;
    }
  ...
  void publish_all_tfs(double pitch_angle,
    ...
    tf_abs.header.frame_id = "bebop_link";
    ...
    tf_cam.header.frame_id = "bebop_link";
